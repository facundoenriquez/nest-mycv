...
describe('UsersController', () => {
  ...
  beforeEach(async () => {
    fakeUsersService = {
      findOne: (id: number) => { return Promise.resolve({ id, email: 'asd@asd.com', password: 'asd' } as User) },
      find: (email: string) => { return Promise.resolve([{ id: 1, email, password: 'asd' } as User]) },
      remove: (id: number) => { return Promise.resolve({ id, email: 'asd@asd.com', password: 'asd' } as User) },
      update: (id: number, attrs: Partial<User>) => { return Promise.resolve({ id, ...attrs } as User) },
    }
    fakeAuthService = {
